...
export default function CustomTextArea({label, ...props}) {
  ...
  return (
      <div id="TextfieldUpdated" className='position-relative'>
        <div className="mb_4 d-flex space-between">
          <label className='Body14M fs-14 fw-5 control-label' style={{color: props.helperText ? ColorSchemeCode.danger30 : focus ? ColorSchemeCode.primary50 : ColorSchemeCode.neutral80, letterSpacing : 0.1,transition:'ease-in-out 0.2s'}}>{label}</label>
        </div>
        <textarea 
